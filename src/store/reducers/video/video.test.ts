...
describe('video reducer', () => {
  ...
  describe(VIDEO__ADD, () => {
    ...
    it('should throw if videoType is invalid', () => {
      ...
      expect(() => video(state, action)).toThrowError(
        `invalid videoType recieved from handleAdd: invalid`,
      )
    })
  })
  describe(VIDEO__REMOVE, () => {
    ...
    it('should throw if videoType is invalid', () => {
      ...
      expect(() => video(state, action)).toThrowError(
        `invalid videoType recieved from handleRemove: invalid`,
      )
    })
  })
  describe(VIDEO__EDIT, () => {
    it('should edit an anime on the list', () => {
      const state = returnMockedState()
      const editedVideo: IVideo = {
        ...animes[0],
        api: { ...animes[0].api, title: 'UPDATED' },
      }
      const action = edit('anime', editedVideo)
      const expected: IVideoState = {
        ...returnMockedState(),
        animes: [editedVideo, animes[1]],
      }
    ...
    it('should edit a movie on the list', () => {
      ...
      const editedVideo: IVideo = {
        ...movies[0],
        api: { ...movies[0].api, title: 'UPDATED' },
      }
      const action = edit('movie', editedVideo)
      const expected: IVideoState = {
        ...returnMockedState(),
        movies: [editedVideo, movies[1]],
      }
    ...
    it('should edit a serie on the list', () => {
      ...
      const editedVideo: IVideo = {
        ...series[0],
        api: { ...series[0].api, title: 'UPDATED' },
      }
      const action = edit('serie', editedVideo)
      const expected: IVideoState = {
        ...returnMockedState(),
        series: [editedVideo, series[1]],
      }
    ...
    it('should throw if videoType is invalid', () => {
      ...
      expect(() => video(state, action)).toThrowError(
        `invalid videoType recieved from handleEdit: invalid`,
      )
    })
  })
  describe(VIDEO__VISIBILITY_FILTER, () => {
    ...
    it('should throw if videoType is invalid', () => {
      ...
      expect(() => video(state, action)).toThrowError(
        `invalid videoType recieved from handleVisibilityFilter: invalid`,
      )
    })
  })
})
